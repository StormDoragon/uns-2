...
import { ArrowLeft, BookOpen, Heart, Shield, Compass } from 'lucide-react'
...
export default async function DuaPage({ params }: Props) {
  ...
  const related = allDuas
    .filter((d) => d.slug !== dua.slug)
    .slice(0, 3)
  ...
  return (
    <div className="min-h-screen bg-[#f8f1e7] dark:bg-[#0a0f0d] text-stone-800 dark:text-stone-100">
      ...
      <main className="max-w-3xl mx-auto px-6 pt-12 pb-24">
        ...
        <section
          aria-label="Transliteration and translation"
          className="grid md:grid-cols-2 gap-4 mb-14 animate-slide-up"
        >
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-2xl p-8">
            <p className="uppercase text-xs tracking-[2px] text-stone-400 mb-3 font-medium">
              Transliteration
            </p>
          ...
          <section className="animate-fade-in">
            ...
            <div className="space-y-2">
              {related.map((r) => (
                ...
                >
                  <div>
                    <p className="text-xs text-emerald-700 dark:text-emerald-500 font-medium mb-0.5">
                      {r.category}
                    </p>
                  ...
                  <span className="text-stone-300 dark:text-stone-600 group-hover:text-emerald-400 transition-colors text-sm">
                    →
                  </span>
                </Link>
              ))}
